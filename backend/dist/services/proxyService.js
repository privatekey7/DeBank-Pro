...
class ProxyService {
    constructor() {
        ...
        this.loadProxies = () => {
            try {
                const proxyFilePath = path_1.default.join(process.cwd(), '..', 'proxy.txt');
                if (!fs_1.default.existsSync(proxyFilePath)) {
                    this.logger.warn('Файл proxy.txt не найден. Прокси не будут использоваться.');
                    return;
                }
                const proxyContent = fs_1.default.readFileSync(proxyFilePath, 'utf-8');
