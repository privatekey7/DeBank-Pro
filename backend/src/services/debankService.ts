...
export class DeBankService {
    ...
    while (retries < this.maxRetries) {
      ...
      try {
        ...
        if (error instanceof Error && (
          error.message.includes('ERR_INVALID_AUTH_CREDENTIALS') || 
  ...
  private scrapeWalletData = async (walletAddress: string, proxy: ProxyConfig | null): Promise<WalletData | null> => {
  ...
  private processWalletData = (walletAddress: string, networkData: DeBankApiResponse): WalletData => {
