...
## 🔀 Источник баланса (Rabby)

Балансы берутся из **Rabby API** (`api.rabby.io`): итог кошелька приходит
готовым агрегатом в одном запросе `total_balance`, поэтому ручного
суммирования нет — это устраняет «фантомные» балансы (когда под нагрузкой
DeBank отдавал портфель чужого адреса и итог раздувался). Ветка DeBank
удалена (как в github.com/privatekey7/DeBankChecker, PR #5).

Заголовки запросов повторяют клиент Rabby байт-в-байт (сверено с HAR
расширения): lowercase-подписные заголовки, `x-api-time` = время выдачи
ключа, `x-version: 0.94.2`, браузерные `sec-fetch-*`/`accept-language`.
Отклонение карается фейковым 429 с пустым телом — разбор инцидента:
`docs/incident-429-antibot.md` в репозитории DeBankChecker.
